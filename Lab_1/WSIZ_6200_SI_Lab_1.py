...
#stworzenie funcji pobierającej dane i konversja do json.
def currencyDef(currency,beg,end):
...
rate1 = currencyDef(dolar,date_from,date_to)
rate2 = currencyDef(euro,date_from,date_to)
...
plt.ylim(ymin=0)
plt.title('Korelacja {} do {} = {}'.format(dolar, euro, correlation))
plt.ylim(3.5, 5)
plt.ylabel('Wartość w PLN')
plt.xlabel('Data')
plt.legend([dolar, euro], loc='lower right')
plt.show()